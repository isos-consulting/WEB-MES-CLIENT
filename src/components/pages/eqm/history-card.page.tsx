...
export const PgEqmHistoryCard = () => {
  ...
  const [historyGridPopupCreateVisible, setHistoryGridPopupCreateVisible] =
    useState<boolean>(false);
  const [historyGridPopupUpdateVisible, setHistoryGridPopupUpdateVisible] =
    useState<boolean>(false);
        ...
        options: {
          ...
          onClick: (ev, props) => {
            onSetHistoryInfo(
              ...
              inputRefHistory?.current?.setValues(
                props?.grid?.store?.data?.rawData[props?.rowKey],
              );
            });
          },
  ...
  const getHistoryData = () => {
    getData(
      { equip_uuid: historyInfo?.equip_uuid },
      URL_PATH_EQM.HISTORY.GET.HISTORIES,
    ).then(res => {
      setHistoryData(res);
    });
  };
  ...
  useLayoutEffect(() => {
    if (
      (historyGridPopupVisible && !historyGridPopupCreateVisible) ||
      historyGridPopupUpdateVisible
    ) {
      getHistoryData();
    }
  }, [
    historyGridPopupVisible,
    historyGridPopupCreateVisible,
    historyGridPopupUpdateVisible,
  ]);
  ...
  useLayoutEffect(() => {
    if (historyGridPopupCreateVisible === true) {
      inputRefCreateHistory?.current?.setValues(
