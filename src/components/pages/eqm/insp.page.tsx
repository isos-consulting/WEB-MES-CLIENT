...
export const PgEqmInsp = () => {
  ...
  const onApplyInsp = (ev, props) => {
    ...
    executeData(
      ...
      if (success) {
        message.success('기준서가 ' + (value ? '해제' : '적용') + '되었습니다.');
  ...
  const headerGrid = useGrid('HEADER_GRID', [
    {header:'설비UUID', name:'equip_uuid', width:ENUM_WIDTH.L, filter:'text', hidden:true},
    {header:'설비코드', name:'equip_cd', width:ENUM_WIDTH.L, filter:'text'},
    {header:'설비명', name:'equip_nm', width:ENUM_WIDTH.L, filter:'text'},
  ], {
  ...
  const detailSubGrid = useGrid('DETAIL_SUB_GRID', [
    ...
    {header:'주기 기준일', name:'base_date', width:ENUM_WIDTH.M, format:'date', editable:true},
    {header:'일상점검주기UUID', name:'daily_insp_cycle_uuid', width:200, hidden:true, format:'text'},
    {header:'일상점검주기명', name:'daily_insp_cycle_nm', width:80, editable:true, format:'combo'},
    {header:'주기단위UUID', name:'cycle_unit_uuid', width:200, hidden:true, format:'text'},
    {header:'주기단위', name:'cycle_unit_nm', width:80, editable:true, format:'combo'},
    {header:'점검주기', name:'cycle', width:ENUM_WIDTH.M, editable:true, format:'number', filter:'number'},
    {header:'비고', name:'remark', width:ENUM_WIDTH.L, editable:true, filter:'text'},
  ], {
  ...
  const newDataPopupGrid = useGrid('NEW_DATA_POPUP_GRID', 
    ...
    gridComboInfo: [
      { // 투입단위 콤보박스
        columnNames: [
          {codeColName:{original:'daily_insp_cycle_uuid', popup:'daily_insp_cycle_uuid'}, textColName:{original:'daily_insp_cycle_nm', popup:'daily_insp_cycle_nm'}},
        ],
        dataApiSettings: {
      ...
      { // 투입단위 콤보박스
        columnNames: [
          {codeColName:{original:'cycle_unit_uuid', popup:'cycle_unit_uuid'}, textColName:{original:'cycle_unit_nm', popup:'cycle_unit_nm'}},
        ],
        dataApiSettings: {
  ...
  const detailInputInfo = useInputGroup('DETAIL_INPUTBOX', [
    ...
    {type:'text', id:'equip_cd', label:'설비코드', disabled:true, hidden:true},
    {type:'text', id:'equip_nm', label:'설비명', disabled:true, hidden:true},
  ]);
  ...
  const detailSubInputInfo = useInputGroup('DETAIL_SUB_INPUTBOX', [
    {type:'text', id:'insp_uuid', alias:'uuid', label:'기준서UUID', disabled:true, hidden:true},
    {type:'text', id:'equip_uuid', label:'설비UUID', disabled:true, hidden:true},
    {
      type:'text', id:'equip_cd', label:'설비코드', disabled:true, usePopup:true,
    ...
    dataGridEvents.onSave('headerInclude', {
      ...
      async (res) => {
        ...
        onSearchHeader(headerSearchInfo?.values).then(async (searchResult) => {
          await onAfterSaveAction(searchResult, headerRow?.equip_uuid, detailRow?.insp_uuid);
        });
      },
      true,
    );
  ...
  const onAfterSaveAddData = async (isSuccess, savedData?) => {
    ...
    // 헤더 그리드 재조회
    onSearchHeader(headerSearchInfo?.values).then((searchResult) => { onAfterSaveAction(searchResult, headerRow?.equip_uuid, detailRow?.insp_uuid); });
  ...
  const onAfterSaveAction = async (searchResult, header_uuid, detail_uuid) => {
    let selectedHeaderRow = searchResult?.find(el => el?.equip_uuid === header_uuid);
