...
export const PgQmsRework = () => {
  ...
  const newDataPopupInputInfo = useInputGroup('NEW_DATA_POPUP_INPUT_BOX', [
    ...
    {
      type:'combo', id:'rework_type_cd', label:'일괄처리기준', default:'DISPOSAL', firstItemType:'none',
      // dataSettingOptions: {
  ...
  const grid = useGrid('HEADER_GRID', [
    ...
    {header:'부적합', name:'reject_nm', width:ENUM_WIDTH.M, format:'popup', filter:'text', requiredField:true},
    {header:'LOT NO', name:'lot_no', width:ENUM_WIDTH.M, filter:'text', requiredField:true},
    {header:'부적합판정 수량', name:'qty', width:ENUM_WIDTH.M, format:'number', filter:'number', editable:true, requiredField:true},
    {header:'부적합판정 코드', name:'rework_type_cd', width:ENUM_WIDTH.M, hidden:true, requiredField:true},
    {header:'부적합판정', name:'rework_type_nm', width:ENUM_WIDTH.M, format:'popup', filter:'text', editable:true, requiredField:true},
    {header:'출고창고UUID', name:'from_store_uuid', width:ENUM_WIDTH.M, hidden:true},
    {header:'출고창고', name:'from_store_nm', width:ENUM_WIDTH.M, filter:'text'},
    {header:'출고위치UUID', name:'from_location_uuid', width:ENUM_WIDTH.M, hidden:true},
    ...
    {header:'입고창고', name:'to_store_nm', width:ENUM_WIDTH.M, format:'popup', filter:'text', editable:true},
    {header:'입고위치UUID', name:'to_location_uuid', width:ENUM_WIDTH.M, hidden:true},
    {header:'입고위치', name:'to_location_nm', width:ENUM_WIDTH.M, format:'popup', filter:'text', editable:true},
    {header:'비교', name:'remark', width:ENUM_WIDTH.L, filter:'text', editable:true},
  ], {
  ...
  const fomulaQty = (fomulaParams, props, columnType:'입고'|'반출') => {
    ...
    const targetValue = Number(fomulaParams?.targetValues[fomulaParams?.targetColumnNames[0]]);
    const maxValue = Number(fomulaParams?.targetValues[fomulaParams?.targetColumnNames[1]]);

    const compareValue = maxValue - (inputValue + targetValue);

    console.log((inputValue + targetValue) > maxValue, inputValue + targetValue , maxValue)
  ...
  //#region 🔶 분해이력 관리
  const disassemblePopupGrid = useGrid('DISASSEMBLE_GRID', [
    {header:'부적합품판정분해UUID', name:'rework_disassemble_uuid', width:ENUM_WIDTH.M, hidden:true},
    {header:'품목UUID', name:'prod_uuid', width:ENUM_WIDTH.M, hidden:true},
    {header:'품번', name:'prod_no', width:ENUM_WIDTH.M, filter:'text', requiredField:true},
    {header:'품명', name:'prod_nm', width:ENUM_WIDTH.L, filter:'text', requiredField:true},
    {header:'Rev', name:'rev', width:ENUM_WIDTH.M, filter:'text'},
    ...
    {header:'단위UUID', name:'unit_uuid', width:ENUM_WIDTH.M, hidden:true},
    {header:'단위', name:'unit_nm', width:ENUM_WIDTH.S, filter:'text'},
    {header:'소요량', name:'usage', width:ENUM_WIDTH.M, format:'number', filter:'number'},
    {header:'기준수량', name:'qty', width:ENUM_WIDTH.M, format:'number', filter:'number'},
    {header:'재입고 LOT NO', name:'lot_no', width:ENUM_WIDTH.M, filter:'text', editable:true, requiredField:true},
    {
      header:'입고수량', name:'income_qty', width:ENUM_WIDTH.M, format:'number', filter:'number', defaultValue:0, editable:true,
      formula: {
        targetColumnNames: ['return_qty', 'qty'],
        resultColumnName: 'disposal_qty',
        formula: (fomulaParams, props) => fomulaQty(fomulaParams, props, '입고'),
      }
    },
    {header:'입고창고UUID', name:'income_store_uuid', width:ENUM_WIDTH.M, hidden:true},
    {header:'입고창고', name:'income_store_nm', width:ENUM_WIDTH.M, format:'popup', filter:'text', editable:true},
    {header:'입고위치UUID', name:'income_location_uuid', width:ENUM_WIDTH.M, hidden:true},
    {header:'입고위치', name:'income_location_nm', width:ENUM_WIDTH.M, format:'popup', filter:'text', editable:true},
    {
      header:'반출수량', name:'return_qty', width:ENUM_WIDTH.M, format:'number', filter:'number', defaultValue:0, editable:true,
      formula: {
        targetColumnNames: ['income_qty', 'qty'],
        resultColumnName: 'disposal_qty',
        formula: (fomulaParams, props) => fomulaQty(fomulaParams, props, '반출'),
      }
    },
    {header:'반출창고UUID', name:'return_store_uuid', width:ENUM_WIDTH.M, hidden:true},
    {header:'반출창고', name:'return_store_nm', width:ENUM_WIDTH.M, format:'popup', filter:'text', editable:true},
    {header:'반출위치UUID', name:'return_location_uuid', width:ENUM_WIDTH.M, hidden:true},
    {header:'반출위치', name:'return_location_nm', width:ENUM_WIDTH.M, format:'popup', filter:'text', editable:true},
    {header:'폐기수량', name:'disposal_qty', width:ENUM_WIDTH.M, format:'number', filter:'number', defaultValue:0},
    {header:'비교', name:'remark', width:ENUM_WIDTH.L, filter:'text', editable:true},
  ], {
    title: '분해 이력',
  ...
  const extraGridPopups:TExtraGridPopups = [
    {
      ...
      saveOptionParams: changeNameToAlias(disassemblePopupInputInfo?.values, disassemblePopupInputInfo?.inputItems),
