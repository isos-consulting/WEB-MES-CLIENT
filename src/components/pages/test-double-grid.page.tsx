...
export const PgTestDoubleGrid = () => {
      ...
      rowAddPopupInfo: {
        ...
        columns: [
          ...
          },
          {
            ...
            width: ENUM_WIDTH.M,
            hidden: true,
            defaultValue: getToday('YYYYMMDD'),
          },
        ],
        dataApiSettings: () => {
          type TParams = {
      ...
      extraButtons: [
        {
          ...
          buttonAction: (ev, props, options) => {
            ...
            if (newDataPopupGridVisible) {
              params['partner_uuid'] =
                newDataPopupInputInfo.values?.partner_uuid;
            } else if (editDataPopupGridVisible) {
              params['partner_uuid'] =
                editDataPopupInputInfo.values?.partner_uuid;
            } else if (addDataPopupGridVisible) {
              params['partner_uuid'] =
                addDataPopupInputInfo.values?.partner_uuid;
            }
            ...
            getData(params, '/mat/order-details').then(res => {
              modal.confirm({
                title: '발주품목 - 다중선택',
                width: '80%',
                content: (
  ...
  const [customGridPopupVisible, setCustomGridPopupVisible] =
    useState<boolean>(false);
  ...
  const headerExtraButtons = [
    {
      ...
      onClick: () => {
        modal.info({
          ...
          content: (
            <img src="https://item.kakaocdn.net/do/ae897b8fe81a9cf7bcbe9af58d473755617ea012db208c18f6e83b1a90a7baa7" />
          ),
          okText: '확인',
        });
      },
  ...
  const extraModals = [
    {
      ...
      children: (
        <img src="https://item.kakaocdn.net/do/493188dee481260d5c89790036be0e66effd194bae87d73dd00522794070855d" />
      ),
  ...
  const extraGridPopups: TExtraGridPopups = [
    {
      ...
      columns: [
        ...
        /** 🌵셀에 버튼 넣기 사용 예시 */
        {
          ...
          format: 'button',
          width: 200,
          options: { value: '셀을 클릭하면 동작' },
        },
        {
          name: 'test_button2',
          header: '글자고정 + onClick속성사용',
          format: 'button',
          width: 200,
          options: {
            value: '버튼을 누르면 동작',
            onClick: (ev, props) =>
              message.info(
                props.columnInfo.name + ' ' + props.rowKey + '을(를) 눌렀구나',
              ),
          },
        },
        {
