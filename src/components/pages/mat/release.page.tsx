...
export const PgMatRelease = () => {
  ...
  const newDataPopupGridColumns = cloneObject(grid.gridInfo.columns)?.filter(
    el => {
      if (
        !['qty', 'to_store_nm', 'to_location_nm', 'remark'].includes(el?.name)
      )
        el['editable'] = false;
  ...
  //#region 🔶 출고요청 관리
  const releaseRequestPopupGrid = useGrid(
    'RELEASE_REQUEST_GRID',
      ...
      rowAddPopupInfo: {
        ...
        dataApiSettings: () => {
          const params = {
            ...
            price_type: 'all',
            reg_date: dayjs(
              releaseRequestPopupInputInfo?.ref?.current?.values?.reg_date,
            )?.format('YYYY-MM-DD'),
            // zero_except_fg: true,
            // minus_except_fg: true,
            prod_uuid:
              releaseRequestPopupInputInfo?.ref?.current?.values?.prod_uuid,
          };
  ...
  const releaseRequestPopupInputInfo = useInputGroup(
    'ReleaseRequest_INPUTBOX',
    [
      {
        type: 'date',
        id: 'reg_date',
  ...
  useLayoutEffect(() => {
    if (releaseRequestPopupVisible === false) {
      ...
      releaseRequestPopupInputInfo?.setFieldValue(
        'reg_date',
  ...
  const extraGridPopups: TExtraGridPopups = [
    {
      ...
      onCancel: ev => {
        const releaseRequestData =
          releaseRequestPopupGrid?.gridInstance?.getData();

        if (releaseRequestData?.length > 0) {
          modal.confirm({
            title: '추가 취소',
            content:
              '작성중인 항목이 있습니다. 출고요청 불러오기를 취소하시겠습니까?',
      ...
      saveOptionParams: changeNameToAlias(
        releaseRequestPopupInputInfo?.values,
        releaseRequestPopupInputInfo?.inputItems,
      ),
