...
export const PgMatReceive = () => {
  ...
  const detailGrid = useGrid(
    ...
    [
      ...
      },
      {
        ...
        filter: 'text',
        format: 'tag',
        options: { conditions: inspectionCondition },
        editable: false,
      },
      {
        header: '이월',
  ...
  const newDataPopupGrid = useGrid(
    'NEW_DATA_POPUP_GRID',
    (cloneObject(detailGrid.gridInfo.columns) as any[]).filter(el => {
      if (el?.name === 'lot_no') {
        el['defaultValue'] = () => {
  ...
  const reloadDetailGrid = uuid => {
    ...
    getData(detailSearchInfo?.values, uriPath, 'header-details').then(
      (res: any) => {
