...
export const PgInvStoresTypeHistory = () => {
  ...
  const complexColumns: OptComplexColumnInfo[] = useMemo(() => {
    ...
    for (let i = 0; i <= tranTypes.length - 1; i++) {
      _complexColumns.push({
        ...
        childNames: [
          tranTypes[i]?.tran_type_cd + '_in_qty',
          tranTypes[i]?.tran_type_cd + '_out_qty',
        ],
      });
    }
  ...
  useLayoutEffect(() => {
    if (complexColumns) grid?.setComplexColumns(complexColumns);
  }, [complexColumns]);

  useLayoutEffect(() => {
    getData({}, searchTransactionUriPath, 'raws').then(res => {
      setTranTypes(res);
    });
  }, []);
