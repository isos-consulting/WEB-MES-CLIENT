...
export const PgInvMove = () => {
  ...
  const searchUriPath = '/inv/moves';
  const saveUriPath = '/inv/moves';

  /** 그리드 상태를 관리 */
  const grid = useGrid('GRID', [
    {header: '창고이동아이디', name:'move_uuid', width:ENUM_WIDTH.M, alias: 'uuid', filter:'text', format:'text', hidden:true},
    {header: '이동일시', name:'reg_date', width:ENUM_WIDTH.L, filter:'text', format:'date', requiredField:true},
  ...
  const searchInfo = useSearchbox('SEARCH_INPUTBOX', [
    {type:'daterange', id:'reg_date', ids:['start_date', 'end_date'], defaults:[getToday(-7), getToday()], label:'이동일'},
  ...
  const newDataPopupInputInfo = useInputGroup('NEW_DATA_POPUP_INPUT_BOX', [
    {type:'date', id:'reg_date', label:'이동일', default:getToday()},
  ]);
