...
export const PgInvStore = () => {
  ...
  useLayoutEffect(() => {
    // 기준창고 콤보박스 선택 값 지정
    if (!storeUuid) setStoreUuid('all');
    else searchInfo.instance?.setFieldValue('store_uuid', storeUuid);
  }, [storeUuid]);
  ...
  useLayoutEffect(() => {
    const columns: any[] = [
      ...
      {
        header:
          newDataPopupGridVisible || editDataPopupGridVisible
            ? '실사수량'
            : '재고',
        name: 'qty',
        width: ENUM_WIDTH.M,
