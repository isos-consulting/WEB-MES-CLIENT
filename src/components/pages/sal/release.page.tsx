...
export const PgSalRelease = () => {
  ...
  const [orderProdPopupVisible, setOrderProdPopupVisible] =
    useState<boolean>(false);
  const [outgoOrderPopupVisible, setOutgoOrderPopupVisible] =
    useState<boolean>(false);
  ...
  const editDataPopupGridColumns = cloneObject(
    newDataPopupGrid?.gridInfo?.columns,
  )?.map(el => {
    if (!['qty', 'remark'].includes(el?.name)) el['editable'] = false;
  ...
  //#region 🔶 수주폼목 불러오기
  const orderProdPopupGrid = useGrid(
    'ORDER_PROD_GRID',
    cloneObject(grid?.gridInfo?.columns).filter(
      el =>
        ![
          'reg_date',
          'outgo_order_detail_uuid',
          'outgo_order_qty',
          'order_qty',
        ].includes(el?.name),
    ),
      ...
      rowAddPopupInfo: {
        ...
        dataApiSettings: () => {
          const params = {
            ...
            price_type: 'all',
            reg_date: dayjs(
              orderProdPopupInputInfo.ref.current?.values?.reg_date,
            )?.format('YYYY-MM-DD'),
            zero_except_fg: true,
          ...
          return {
            ...
            onInterlock: () => {
              const regDate =
                orderProdPopupInputInfo.ref.current?.values?.reg_date;
              const prodUuid =
                orderProdPopupInputInfo.ref.current?.values?.prod_uuid;
  ...
  const orderProdPopupInputInfo = useInputGroup(
    'ORDER_PROD_INPUTBOX',
    [
      {
        type: 'text',
        id: 'order_detail_uuid',
        label: '수주세부항목UUID',
        disabled: true,
        hidden: true,
      },
  ...
  useLayoutEffect(() => {
    if (orderProdPopupVisible === false) {
      ...
      orderProdPopupInputInfo?.setFieldValue(
        'reg_date',
  ...
  //#region 🔶 출하지시 불러오기
  const outgoOrderPopupGrid = useGrid(
    'OUTGO_ORDER_GRID',
    cloneObject(grid?.gridInfo?.columns).filter(
      el =>
        ![
      ...
      rowAddPopupInfo: {
        ...
        dataApiSettings: () => {
          const params = {
            ...
            price_type: 'all',
            reg_date: dayjs(
              outgoOrderPopupInputInfo.ref.current?.values?.reg_date,
            )?.format('YYYY-MM-DD'),
            zero_except_fg: true,
            minus_except_fg: true,
          ...
          return {
            ...
            onInterlock: () => {
              const regDate =
                outgoOrderPopupInputInfo.ref.current?.values?.reg_date;
              const prodUuid =
                outgoOrderPopupInputInfo.ref.current?.values?.prod_uuid;
  ...
  const outgoOrderPopupInputInfo = useInputGroup(
    'OUTGO_ORDER_INPUTBOX',
    [
      {
        type: 'text',
        id: 'outgo_order_detail_uuid',
        label: '출하지시세부항목UUID',
        disabled: true,
        hidden: true,
      },
  ...
  useLayoutEffect(() => {
    if (outgoOrderPopupVisible === false) {
      ...
      outgoOrderPopupInputInfo?.setFieldValue(
        'reg_date',
        newDataPopupInputInfo.ref.current?.values?.reg_date,
      );
    }
  ...
  const extraGridPopups: TExtraGridPopups = [
    // 🌵수주품목 불러오기 팝업
    {
      ...
      onCancel: ev => {
        ...
        if (releaseRequestData?.length > 0) {
          modal.warning({
            title: '추가 취소',
            content:
              '작성중인 항목이 있습니다. 수주품목 불러오기를 취소하시겠습니까?',
      ...
      onOk: () => {
        ...
        if (releaseRequestData?.length > 0) {
          ...
          releaseRequestData?.map(el => {
            const uuid =
              orderProdPopupInputInfo.ref.current?.values?.order_detail_uuid;
      ...
      saveOptionParams: changeNameToAlias(
        orderProdPopupInputInfo?.values,
        orderProdPopupInputInfo?.inputItems,
      ),
    ...
    // 🌵출하지시 불러오기 팝업
    {
      ...
      onCancel: ev => {
        ...
        if (releaseRequestData?.length > 0) {
          modal.warning({
            title: '추가 취소',
            content:
              '작성중인 항목이 있습니다. 출하지시 불러오기를 취소하시겠습니까?',
      ...
      onOk: () => {
        ...
        if (releaseRequestData?.length > 0) {
          ...
          releaseRequestData?.map(el => {
            const uuid =
              outgoOrderPopupInputInfo.ref.current?.values
                ?.outgo_order_detail_uuid;
      ...
      saveOptionParams: changeNameToAlias(
        outgoOrderPopupInputInfo?.values,
        outgoOrderPopupInputInfo?.inputItems,
      ),
