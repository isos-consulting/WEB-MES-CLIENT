...
export const PgSalReturnReport = () => {
  ...
  const searchInfo = useSearchbox('SEARCH_INPUTBOX', [
    {type:'daterange', id:'reg_date', ids:['start_date', 'end_date'], defaults:[getToday(-7), getToday()], label:'반입일', useCheckbox:true},
  ...
  const columns = useMemo(() => {
    ...
    switch (searchInfo.values?.sort_type) {
      case 'prod':
        _columns = [
          {header: '품목', width:ENUM_WIDTH.L, name:'prod_uuid', filter:'text', hidden:true},
          ...
          {header: '반입일자', width:ENUM_WIDTH.M,name:'reg_date',  filter:'text', format:'date'},
          {header: 'LOT NO', width:ENUM_WIDTH.M, name:'lot_no', filter:'text'},
          {header: '반입수량', width:ENUM_WIDTH.M, decimal:ENUM_DECIMAL.DEC_STCOK, name:'qty', format:'number', filter:'number'},
          ...
          {header: '합계금액', width:ENUM_WIDTH.M, decimal:ENUM_DECIMAL.DEC_PRICE, name:'total_price', format:'number', filter:'number'},
      ...
      default:
        _columns = [
          {header: '거래처', name:'partner_uuid', width:ENUM_WIDTH.L, filter:'text', hidden:true},
  ...
  const subColumns = useMemo(() => {
    ...
    switch (searchInfo.values?.sort_type) {
      ...
      case 'date':
        _columns = [
          {header: '반입일자', name:'reg_date', width:ENUM_WIDTH.M, filter:'text', format:'date'},
          {header: '반입수량', width:ENUM_WIDTH.M, decimal:ENUM_DECIMAL.DEC_STCOK, name:'qty', format:'number', filter:'number'},
          {header: '공급가액', name:'supply_price', width:ENUM_WIDTH.M, decimal:ENUM_DECIMAL.DEC_PRICE, format:'number', filter:'number'},
          {header: '부가세액', name:'tax', width:ENUM_WIDTH.M, decimal:ENUM_DECIMAL.DEC_PRICE, format:'number', filter:'number'},
          {header: '합계금액', name:'total_price', width:ENUM_WIDTH.L, decimal:ENUM_DECIMAL.DEC_PRICE, format:'number', filter:'number'},
