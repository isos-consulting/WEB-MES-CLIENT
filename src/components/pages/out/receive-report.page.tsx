...
export const PgOutReceiveReport = () => {
    ...
    switch (searchInfo.values?.sort_type) {
      case 'prod':
        _columns = [
          ...
          {header: '입고일자', name:'reg_date', width:ENUM_WIDTH.M, filter:'text', format:'date'},
          {header: '입고수량', width:ENUM_WIDTH.M, decimal:ENUM_DECIMAL.DEC_STCOK, name:'income_qty', format:'number', filter:'number'},
  ...
  const setSubTotalDatas = (data:object[]) => {
    if (data?.length > 0) {
      const curculationColumnNames = ['income_qty', 'supply_price', 'tax', 'total_price'];
      const standardNames = (
