...
export const PgAutMenu = () => {
  ...
  detailModalProps.onOk = async _ => {
    ...
    const editChk = () => {
      if (record.uuid == null || record.uuid == '') {
        // [메뉴 이름] 없으면 신규 메뉴 생성으로 판단
        restAPIType = 'post'; //생성
        msg = '생성';
        if (record.parent_uuid == null || record.parent_uuid == '') {
          ...
          alert(
            '메뉴를 ' +
              msg +
              '하시려면 [사용유무] 값 [True/False] 를 입력하세요!',
          );
          return false;
        }
      } else if (
        (record.parent_uuid == null || record.parent_uuid == '') &&
        (record.sortby == null || record.sortby == '')
      ) {
        // [상위메뉴 이름] 과 [정렬순서] 없으면 삭제로 판단
