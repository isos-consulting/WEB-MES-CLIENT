...
export const onDefaultGridSave = async (saveType:'basic'|'headerInclude', ref:MutableRefObject<Grid>, columns, saveUriPath, optionParams, modal, saveAfterFunc?:Function, methodType?, inputInfo?) => {
  // 그리드의 데이터를 편집한 이력이 있는지 체크
  if (isModified(ref, columns) || (optionParams && saveType === 'headerInclude')) { // 편집 이력이 있는 경우
    modal.confirm({
      icon: null,
      title: '저장',
      ...
      onOk: async () => {
        ...
        if (saveType === 'basic') {
          ...
          saveGridData(modifiedRows, columns, saveUriPath, optionParams, false, methodType).then(({success, count, savedData}) => {
            if (saveAfterFunc)
              saveAfterFunc({success, count, savedData});
          });
        ...
        // (header / detail 형식으로 변환 후 저장)
          ...
          // 저장
          await executeData(saveData, saveUriPath, methodType || _methodType).then(({success, count, savedData}) => {
            if (success === true) {
              if (saveAfterFunc)
                saveAfterFunc({success, count, savedData});
            }
...
/** 편집 취소 */
export const onDefaultGridCancel = (ref:MutableRefObject<Grid>, columns, modal, afterCancelFunc?:Function) => {
  // 그리드의 데이터를 편집한 이력이 있는지 체크
