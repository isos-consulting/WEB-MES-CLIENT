...
export const PgPrdReturnReport = () => {
  ...
  const searchInfo = useSearchbox('SEARCH_INPUTBOX', [
    {type:'daterange', id:'reg_date', ids:['start_reg_date', 'end_reg_date'], defaults:[getToday(), getToday()], label:'납입일'},
  ...
  const columns = useMemo(() => {
    ...
    switch (searchInfo.values?.sub_total_type) {
      case 'prod':
        _columns = [
          {header: '품목', name:'prod_id', width:ENUM_WIDTH.L, filter:'text', hidden:true},
      ...
      default:
        _columns = [
          {header: '출고창고', name:'from_store_id', width:ENUM_WIDTH.L, filter:'text', hidden:true},
          {header: '출고창고', name:'from_store_nm', width:ENUM_WIDTH.M, filter:'text'},
          {header: '출고위치', name:'from_location_nm', width:ENUM_WIDTH.M, filter:'text'},
          {header: '반납일자', name:'reg_date', width:ENUM_WIDTH.M, filter:'text', format:'date'},
          {header: '품목유형', width:ENUM_WIDTH.L, name:'item_type_nm', filter:'text'},
