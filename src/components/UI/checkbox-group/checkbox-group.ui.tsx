...
const CheckboxGroup: React.FC<Props> = (props) => {
  const [checkboxGroupValues, setCheckboxGroupValues] = useRecoilState(afAnyArrayState(props.id)); // 체크박스 배열
  const [allCheckValue, setAllCheckValue] = useRecoilState(afObjectState(props.id)); //전체 체크하는 체크박스
  ...
  const onChangeAll = (e?: React.ChangeEvent<HTMLInputElement>) => {
    setCheckboxGroupValues(e?.target.checked ? props.options.map(value => value.code) as string[] : [] as string[]);
    setAllCheckValue({
      ...
      <Space size={10} wrap>
        ...
        <AntCheckbox.Group 
          ...
          {props.allCheckedable ?
            <Checkbox
              indeterminate={allCheckValue['indeterminate']} onChange={onChangeAll} checked={allCheckValue['checked']}
