...
/** 데이터 그리드 속성 인터페이스 */
interface IAllowedDatagridProps
  extends Omit<IDatagridProps, 'gridId' | 'columns'> {
  /** 그리드 아이디 */
  gridId?: string;
  /** 그리드 컬럼 */
  columns?: IGridColumn[];
...
export const gridModel = (props: {
  ...
  /** 그리드 컬럼 재설정  */
  setGridColumns: React.Dispatch<React.SetStateAction<IGridColumn[]>>;

  /** 그리드 디스플레이 여부 재설정 */
  setGridHidden: React.Dispatch<React.SetStateAction<boolean>>;

  /** 헤더 컬럼 병합 세팅 값 */
  setComplexColumns: React.Dispatch<
    React.SetStateAction<OptComplexColumnInfo[]>
  >;
  // setGridHeader
...
export const useGrid = (
  gridId,
  columns: IGridColumn[],
  options?: IAllowedDatagridProps,
) => {
  const [gridMode, setGridMode] = useState<TGridMode>(
    options?.gridMode ?? 'view',
  );
