...
export const inputGroupModel = (props: {
  ...
  props: IInputGroupboxProps;

  /** 그룹입력상자의 값을 가지고 있는 객체 변수 */
  values: FormikValues;

  setValues: SetterOrUpdater<FormikValues>;

  modifiedValues: FormikValues;
...
export const useInputGroup = (
  ...
  const [_inputItems, setInputItems] =
    useState<IInputGroupboxItem[]>(inputItems);
  const [_inputGroupOptions, setInputGroupOptions] =
    useState<Omit<IInputGroupboxProps, 'id' | 'inputItems' | 'innerRef'>>(
      inputGroupOptions,
    );
...
const getModifiedInputGroupValues = (
  id: string,
  oldValues,
  newValues,
): FormikValues => {
  ...
  keys?.forEach(key => {
    if (oldKeys?.includes(key) === false && newKeys?.includes(key)) {
      // 새로 신규 키가 삽입되어 있는 경우
      result[key] = newValues[key];
    } else if (oldKeys?.includes(key) && newKeys?.includes(key)) {
      // 둘다 키를 가지고 있지만 값이 다른 경우
