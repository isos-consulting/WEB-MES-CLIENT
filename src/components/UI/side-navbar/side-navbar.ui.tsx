...
const SideNavbar: React.FC<Props> = props => {
  ...
  const onChangeLevel1State = (selectedLevel1: string, route?: boolean) => {
    consoleLogLocalEnv(
      '%c1레벨 메뉴 클릭 이벤트 테스트 시작',
      'color: green; font-size: 20px;',
    );
    consoleLogLocalEnv(
      `인자 값 테스트 selectedLevel1: ${selectedLevel1}, route: ${route}`,
    );
    setMenuState(prevState => {
      consoleLogLocalEnv(`변경 전 스테이트 :`, prevState);
      return {
        //선택한 Level1에 따른 Level2메뉴 세팅
        ...prevState,
        selectedLevel1,
      };
    });
    ...
    if (route === true) {
      //1Level에서 라우트를 사용하는 경우 2Level 메뉴 숨김
      ...
      setLayoutState(prevState => {
        return {
          ...prevState,
          leftSpacing:
            layoutState.leftSpacing === LEVEL1_Width
              ? LEVEL1_Expanded_Width
              : prevState.leftSpacing, //1Level 선택시 2Level 접혀있으면 자동 펼침
          showResizeBtn: true,
        };
      });
    }
  ...
  return (
    <ScContainer {...props}>
      ...
      {layoutState.showResizeBtn === false ? (
        ...
        <ScResizeButton
          className={layoutState.leftSpacing === LEVEL1_Width ? 'active' : ''}
          onClick={() => {
            setLayoutState(prevState => ({
              ...prevState,
              leftSpacing:
                prevState.leftSpacing === LEVEL1_Width
                  ? LEVEL1_Expanded_Width
                  : LEVEL1_Width,
            }));
          }}
      ...
      {menuRawData?.map((item, index) => (
        <ScMenuItem
          ...
          onClick={() => {
            onChangeLevel1State(
              item.menu_uri,
              item.menu_type === 'page' ? true : false,
            );
          }}
          className={`level1 ${
            item.menu_uri === selectedLevel1 ? 'active' : ''
          }`}
        >
          {item.menu_type === 'page' ? (
...
//--- [2레벨 메뉴 세팅] ----------------------------------------------------------------------
const Level2: React.FC<ILevel2Props> = ({ active }) => {
