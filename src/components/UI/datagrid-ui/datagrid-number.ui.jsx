...
export class DatagridNumberEditor {
  constructor(props) {
      ...
      rootDiv.value = props.value == null ? props.value : String(Number(props?.value).toFixed(decimal));
  ...
  getValue() {
    ...
    if (value < this.state?.min) {
      message.error(`입력하신 숫자가 작습니다. ${this.state?.min} 이상의 숫자로 입력해주세요.`);
      return this.state?.min;
    }

    if (value > this.state?.max) {
      message.error(`입력하신 숫자가 큽니다. ${this.state?.max} 이하의 숫자로 입력해주세요.`);
  ...
  render(props) {
      if (props?.value == null) {
        ...
        if (isNumber(props?.value || null)) {
            this.el.innerText = setNumberToDigit(Number(props?.value).toFixed(this.state?.decimal)) + this.state?.unit;
        } else {
            this.el.innerText = null;
        }
      }
