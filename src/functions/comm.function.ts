...
const getTenantInfo = () => {
  return {
    'restrict-access-to-tenants': getStorageValue({
...
export async function getData<T = any[]>(
  ...
  try {
    ...
    if (
      error?.response?.data?.state?.state_no === errorState.EXPIRED_ACCESS_TOKEN
    ) {
      const refreshState = await getAccessToken();

      if (
        !refreshState ||
        refreshState?.state_no === errorState.EXPIRED_REFRESH_TOKEN
      ) {
        return;
      }
      ...
      error?.response?.data?.state?.state_no === errorState.NOT_FOUND_USER
    ) {
      await setLogout();
    ...
    if (
      datas?.data?.datas?.value?.count === 0 &&
      options?.disabledZeroMessage !== true
    ) {
      message.warning(
        `${
...
export const getMenus = async () => {
  ...
  await getData({}, '/aut/menus/permission')
    ...
    .finally(() => {
      ...
      if (
        getStorageValue({ storageName: 'userInfo', keyName: 'super_admin_fg' })
      ) {
        rawData.unshift({
          component_nm: null,
          create_fg: null,
          ...
          sub_menu: [
            ...
            {
              component_nm: 'PgAdmExcelUploadType',
              create_fg: true,
              delete_fg: true,
              first_menu_uuid: 'adm',
              icon: null,
              ...
              menu_nm: '엑셀업로드유형',
              menu_type: 'page',
              menu_uri: '/adm/excel-upload-type',
              menu_uuid: 'admExcelUploadType',
              read_fg: true,
              sub_menu: [],
              update_fg: true,
            },
            {
              component_nm: 'PgAdmInterfaceState',
              create_fg: true,
              delete_fg: true,
              first_menu_uuid: 'adm',
              icon: null,
              ...
              menu_nm: '인터페이스 가동상태',
              menu_type: 'page',
              menu_uri: '/adm/interface-state',
              menu_uuid: 'admInterfaceState',
              read_fg: true,
              sub_menu: [],
              update_fg: true,
            },
